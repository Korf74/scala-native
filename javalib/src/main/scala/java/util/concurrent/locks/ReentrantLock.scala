...
class ReentrantLock extends Lock with java.io.Serializable {
  ...
  private final var sync: Sync = new NonfairSync()
  ...
  final def getQueueLength = sync.getQueueLength
  ...
  def hasWaiters(condition: Condition): Boolean = {
    ...
    sync.hasWaiters(condition.asInstanceOf[AbstractQueuedSynchronizer.ConditionObject])
  }
  ...
  def getWaitQueueLength(condition: Condition) = {
    ...
    if(!(condition.isInstanceOf[AbstractQueuedSynchronizer.ConditionObject]))
      throw new IllegalArgumentException("not owner")
    sync.getWaitQueueLength(condition.asInstanceOf[AbstractQueuedSynchronizer.ConditionObject])
  }
  ...
  protected def getWaitingThreads(condition: Condition): java.util.Collection[Thread] = {
    ...
    if(!(condition.isInstanceOf[AbstractQueuedSynchronizer.ConditionObject]))
      throw new IllegalArgumentException("not owner")
    sync.getWaitingThreads(condition.asInstanceOf[AbstractQueuedSynchronizer.ConditionObject])
  }
...
object ReentrantLock {
  ...
  abstract class Sync extends AbstractQueuedSynchronizer {
    ...
    override protected final def isHeldExclusively = getExclusiveOwnerThread == Thread.currentThread()
