  ...
  private def getThrowableException: Throwable = {
    ...
    if(false && e.thrower != Thread.currentThread().getId) {
      ...
      try {
        ...
        val cs: Array[Constructor[_]] = ec.getConstructors() // public ctors only
        var i: Int = 0
        while(i < cs.length) {
...
object ForkJoinTask {
  ...
  def inForkJoinPool: Boolean = Thread.currentThread().isInstanceOf[ForkJoinWorkerThread]
  ...
  final class AdaptedRunnableAction(val runnable: Runnable) extends ForkJoinTask[Void] with RunnableFuture[Void] {
  ...
  def adapt(runnable: Runnable): ForkJoinTask[_] = new AdaptedRunnableAction(runnable)

  def adapt[T](runnable: Runnable, result: T): ForkJoinTask[T] = new AdaptedRunnable[T](runnable, result)

  def adapt[T](callable: Callable[_ <: T]): ForkJoinTask[T] = new AdaptedCallable[T](callable)
